...
service.getMetadata = async (urn) => {
  const { access_token } = await service.getInternalToken();
  // Liste les "Model Views" (viewables) => équivalent de GET /{urn}/metadata
  return await modelDerivativeClient.getModelViews(access_token, urn);
};


service.listBuckets = async () => {
  const { access_token } = await service.getInternalToken();
  const resp = await ossClient.getBuckets(access_token, { limit: 64 });
  return resp.items || [];
};


// 🔹 NOUVEAU: properties d’une vue (GUID)
service.getViewProperties = async (urn, guid) => {
  const { access_token } = await service.getInternalToken();
  // (Astuce: pour forcer un refresh côté service, ajoute un header If-Modified-Since très ancien)
  return await modelDerivativeClient.getModelViewProperties(access_token, urn, guid);
};
